...
#include "modulab_mission_msgs/action/back_up.hpp"
#include "nav2_util/node_utils.hpp"

namespace nav2_behaviors
{
  using BackUpAction = modulab_mission_msgs::action::BackUp;
  /**
   * @class nav2_behaviors::BackUp
   * @brief An action server Behavior for backing up
   */
  ...
  public:
    ...
    Status onRun(const std::shared_ptr<const BackUpAction::Goal> command) override
    {
      ...
      collision_offset_ = command->collision_offset;
    ...
    Status onCycleUpdate()
    {
      ...
      if (distance >= collision_offset_){
        if (!isCollisionFree(distance, cmd_vel.get(), pose2d))
        {
          this->stopRobot();
          RCLCPP_WARN(this->logger_, "Collision Ahead - Exiting BackUp");
          return Status::FAILED;
        }
      }
