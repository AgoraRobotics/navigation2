...
: Recovery<MoveForwardAction>(),
  feedback_(std::make_shared<MoveForwardAction::Feedback>())
{
}
...
Status MoveForward::onRun(const std::shared_ptr<const MoveForwardAction::Goal> command)
{
  if (command->target.y != 0.0 || command->target.z != 0.0) {
    RCLCPP_INFO(
      logger_,
      "Moving forward in Y and Z not supported, will only move in X.");
  }
...
Status MoveForward::onCycleUpdate()
{
  ...
  if (!isCollisionFree(distance, cmd_vel.get(), pose2d)) {
    stopRobot();
    RCLCPP_WARN(logger_, "Collision Ahead - Exiting MoveForward");
    return Status::FAILED;
  }
...
bool MoveForward::isCollisionFree(
...
PLUGINLIB_EXPORT_CLASS(nav2_recoveries::MoveForward, nav2_core::Recovery)